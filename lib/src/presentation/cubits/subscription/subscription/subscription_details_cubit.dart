...
class SubscriptionCubit extends Cubit<SubscriptionState> {
  ...
  Future<void> loadSubscriptionDetails(String subscriptionId) async {
    ...
    final result = await _subscriptionUseCase.getSubscriptionById(subscriptionId);
    ...
    result.fold(
      ...
      (_) {
        ...
        emit(SubscriptionActionSuccess(
          action: 'pause',
          message: 'Your subscription has been paused until ${_formatDate(untilDate)}'
        ));
  ...
  void _moveToStage(int stage) {
    ...
    switch (_currentStage) {
      case 0: // Package selection
        emit(SubscriptionCreationStage(
          stage: 0,
  ...
  Future<void> createSubscription() async {
    if (_packageId == null || _mealSlots == null || _mealSlots!.isEmpty || _addressId == null) {
      emit(SubscriptionError(message: 'Missing required information for subscription'));
      return;
    }
    ...
    result.fold(
      (failure) {
        ...
        emit(SubscriptionError(message: 'Failed to create subscription. Please try again.'));
      },
      (subscription) {
